...
from .auth import IntegrationAuthService
from .middleware import AuthMiddleware, RateLimitMiddleware, LoggingMiddleware
from .routes import RunCancellationService, router
...
def create_app(config: Dict = None) -> FastAPI:
    ...
    app.state.auth_service = config.get(
        "auth_service",
        IntegrationAuthService(),
    )
    app.state.run_cancellation_service = config.get(
