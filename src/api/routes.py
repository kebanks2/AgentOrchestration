...
from src.api.agent_service import AgentApiService, ApiValidationError
...
async def list_agents(
    ...
    return _service_response(
        lambda service: service.list_agents(status, group)
    )
...
async def register_agent(
    ...
    return _service_response(
        lambda service: service.register_agent(name, agent_type, config)
    )


@router.get("/agents/count")
async def agent_count():
    return _service_response(lambda service: service.count_agents())


@router.get("/agents/{agent_id}")
async def get_agent(agent_id: str):
    return _service_response(lambda service: service.get_agent(agent_id))


@router.delete("/agents/{agent_id}")
async def delete_agent(agent_id: str):
    return _service_response(lambda service: service.delete_agent(agent_id))


@router.post("/agents/{agent_id}/start")
async def start_agent(agent_id: str):
    return _service_response(lambda service: service.start_agent(agent_id))


@router.post("/agents/{agent_id}/stop")
async def stop_agent(agent_id: str):
    return _service_response(lambda service: service.stop_agent(agent_id))
